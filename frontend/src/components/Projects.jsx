...
const visualMockMap = {
  ...
  2: {
    icon: FaShoppingBag,
    gradient: "from-[#FF9100]/20 to-[#E8DCC8]/20",
    bgPattern: "radial-gradient(circle, rgba(37,99,235,0.1) 0%, transparent 60%)",
    accentColor: "border-[#FF9100]/30 text-[#FF9100] bg-[#FF9100]/5"
  },
  3: {
    icon: FaComments,
    gradient: "from-[#0B2C1F]/20 to-[#34D399]/20",
    bgPattern: "radial-gradient(circle, rgba(6,182,212,0.1) 0%, transparent 60%)",
    accentColor: "border-[#0B2C1F]/30 text-[#0B2C1F] bg-[#0B2C1F]/5"
  },
  4: {
    icon: FaBriefcase,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, index) => {
            const visual = visualMockMap[project.id] || {
              icon: FaProjectDiagram,
              gradient: "from-[#FF9100]/20 to-cyan-500/20",
              bgPattern: "",
              accentColor: "border-[#FF9100]/30 text-[#FF9100] bg-[#FF9100]/100/5"
            };
            ...
            return (
              <motion.div
                ...
                className="group flex flex-col h-full bg-slate-50 border border-[#FF9100]/40 rounded-3xl overflow-hidden transition-all duration-300 hover:shadow-lg hover:border-[#FF9100] hover:bg-white"
              >
                {/* Visual Thumbnail representation */}
                <div 
                  className="w-full aspect-[1.8/1] relative flex items-center justify-center overflow-hidden border-b border-slate-200/40"
                >
                  {project.thumbnail ? (
                    <img 
                    ...
                    <div 
                      className={`absolute inset-0 flex items-center justify-center w-full h-full bg-gradient-to-tr ${visual.gradient}`}
                  ...
                  <span className="absolute top-4 right-4 font-mono text-[9px] font-bold text-slate-500 bg-white/80 border border-slate-200/50 rounded-full px-2 py-0.5 backdrop-blur-sm z-10">
                ...
                <div className="p-6 flex-grow flex flex-col justify-between text-left">
                  <div>
                    {/* Project Title */}
                    <h3 className="font-poppins font-bold text-lg text-[#0B2C1F] group-hover:text-[#FF9100] transition-colors duration-200 mb-3">
                      {project.title}
                    </h3>
                  ...
                  <div className="flex items-center gap-3 border-t border-slate-100 pt-4 mt-auto">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-1 inline-flex items-center justify-center gap-2 font-inter text-xs font-semibold text-white bg-[#FF9100] hover:bg-[#E08000] rounded-lg py-2.5 transition-all active:scale-97 shadow-sm shadow-blue-500/5 hover:shadow-blue-500/15"
                    >
